...
void AHamoniaCharacter::CheckForInteractables()
{
	if (CurrentInteractableActor && IsValid(CurrentInteractableActor))
	{
		...
		APedestal* OldPedestal = Cast<APedestal>(CurrentInteractableActor);
		if (OldPedestal && OldPedestal->GetPlacedObject())
		{
			UOutlineComponent* OldPlacedOutline = OldPedestal->GetPlacedObject()->FindComponentByClass<UOutlineComponent>();
			if (OldPlacedOutline)
			{
				OldPlacedOutline->HideOutline();
			}
		}
	}
	...
	if (bIsLookingAtInteractable && CurrentInteractableActor)
	{
		...
		APedestal* NewPedestal = Cast<APedestal>(CurrentInteractableActor);
		if (NewPedestal)
		{
			AActor* PlacedObject = NewPedestal->GetPlacedObject();
			if (PlacedObject)
			{
				UOutlineComponent* PlacedOutline = PlacedObject->FindComponentByClass<UOutlineComponent>();
				if (PlacedOutline)
				{
					PlacedOutline->ShowOutline();
				}
			}
		}
	}
